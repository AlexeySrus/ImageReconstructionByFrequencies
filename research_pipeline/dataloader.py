...
def aa_crop(image_a: np.ndarray, image_b: Optional[np.ndarray], window_size: int) -> Tuple[np.ndarray, np.ndarray]:
    min_img_size = min(image_a.shape[:2])
    max_scale = min_img_size // window_size
    assert max_scale > 2, "Small max_scale value: {}".format(max_scale)
    # x_scale = np.random.randint(2, max_scale + 1)
    x_scale = get_random_value_from_interval(2, max_scale)

    noisy_image, clear_image = random_crop_with_transforms(
        image_a.copy(), 
        image_a.copy() if image_b is None else image_b.copy(),
        window_size=int(window_size * x_scale),
        random_swap=False
    )
    ...
    noisy_image = cv2.resize(noisy_image, (window_size, window_size), interpolation=cv2.INTER_NEAREST)
    clear_image = cv2.resize(clear_image, (window_size, window_size), interpolation=cv2.INTER_AREA)
    ...
    def __getitem__(self, _idx: int) -> Union[Tuple[torch.Tensor, torch.Tensor], Tuple[torch.Tensor, torch.Tensor, str]]:
        ...
        if self.need_crop:
            if self.aa_simulation and np.random.rand() < 0.1:
                noisy_image, clear_image = aa_crop(noisy_image, clear_image, self.window_size)
            else:
                noisy_image, clear_image = random_crop_with_transforms(
...
class SyntheticNoiseDataset(Dataset):
    ...
    def __getitem__(self, _idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.aa_simulation and min(clear_image.shape[:2]) > int(self.window_size * 2.5) and np.random.rand() < 0.1:
            clear_crop, noisy_crop = aa_crop(clear_image, None, self.window_size)
        else:
        ...
        if np.random.randint(1, 101) > SYNTH_CONFIG['ADD_NOISE']:
            if np.random.randint(1, 101) > SYNTH_CONFIG['GAUSS_AND_POISSON_NOISE']:
                if np.random.randint(1, 101) > SYNTH_CONFIG['POISSON_NOISE']:
                    noisy_crop = generate_additive_poisson_noise(noisy_crop)
                else:
                    std = np.random.uniform(1, 90)
                    use_fft_noise = self.grayscale and np.random.choice([False, False, False, True])
                    noisy_crop = generate_additive_gaussian_noise(noisy_crop, std, use_fft_noise)
            else:
                noisy_crop = self.noise_transform(image=noisy_crop)['image']
...
if __name__ == '__main__':
    ...
    dataset = SyntheticNoiseDataset(
        "/media/alexey/SSDData/datasets/denoising_dataset/base_clear_images/",
        window_size=256,
        aa_simulation=True
    )
