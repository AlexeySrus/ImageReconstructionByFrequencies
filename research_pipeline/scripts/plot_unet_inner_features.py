...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        pred, inner_features = model.forward(input_tensor.to(device))

    pred = torch.clamp(pred, 0, 1)
    pred_image = tensor_to_image(pred[0].to('cpu'))
    inner_features = torch.cat(inner_features, dim=1).to('cpu')
    inner_features = tensor_to_image(inner_features[0])
