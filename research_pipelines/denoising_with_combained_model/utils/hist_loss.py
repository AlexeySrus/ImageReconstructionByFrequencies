  ...
  def forward(self, x):
    ...
    if x.shape[2] > self.insz or x.shape[3] > self.insz:
      if self.resizing == 'interpolation':
        x_sampled = torch.nn.functional.interpolate(x, size=(self.insz, self.insz),
...
class HistLoss(torch.nn.Module):
    def __init__(self, h=64, image_size=512, resizing='interpolation',
               method='inverse-quadratic', sigma=0.02, intensity_scale=True,
               device='cuda'):
        super().__init__()

        self.histogram_block = RGBuvHistBlock(h, image_size, resizing, method, sigma, intensity_scale, device)
