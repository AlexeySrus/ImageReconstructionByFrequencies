...
def wavelet_wrapper(in_ch: int, out_ch: int = 4*3):
...
class FFTAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        return out, [complex_sa, float_sa]
...
class FFTAttentionUNetModule(nn.Module):
    def __init__(self, in_ch: int, mid_ch: int, out_ch: int, need_up_features: bool = False):
        ...
        self.upsample4 = FeaturesUpsample(mid_ch * 4, mid_ch * 3)
        self.upsample3 = FeaturesUpsample(mid_ch * 3, mid_ch * 2)
        ...
        self.wavelets_1 = wavelet_wrapper(mid_ch)
        self.wavelets_2 = wavelet_wrapper(mid_ch * 2)
        self.wavelets_3 = wavelet_wrapper(mid_ch * 3)
        self.wavelets_4 = wavelet_wrapper(mid_ch * 4)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, List[torch.Tensor], List[torch.Tensor]]:
        ...
        init_deep_f, sa_f = self.deep_conv_block(down_f4)

        deep_f, sa_up_4 = self.upsample4(init_deep_f)
        decoded_f4 = torch.cat((down_f3, deep_f), axis=1)
        init_decoded_f4, sa_df4 = self.upsample_features_block4(decoded_f4)

        deep_f, sa_up_3 = self.upsample3(init_decoded_f4)
        decoded_f3 = torch.cat((down_f2, deep_f), axis=1)
        init_decoded_f3, sa_df3 = self.upsample_features_block3(decoded_f3)

        deep_f, sa_up_2 = self.upsample2(init_decoded_f3)
        decoded_f2 = torch.cat((down_f1, deep_f), axis=1)
        init_decoded_f2, sa_df2 = self.upsample_features_block2(decoded_f2)

        decoded_f2, sa_up_1 = self.upsample1(init_decoded_f2)
        decoded_f1 = torch.cat((hx, decoded_f2), dim=1)
        decoded_f1, sa_df1 = self.upsample_features_block1(decoded_f1)

        features_list = [
            self.wavelets_1(init_decoded_f2), 
            self.wavelets_2(init_decoded_f3), 
            self.wavelets_3(init_decoded_f4), 
            self.wavelets_4(init_deep_f)
        ]
        attn_list  = sa_init + sa_f1 + sa_f2 + sa_f3 + sa_f4 + sa_f + sa_up_4 + sa_up_3 + sa_up_2 + sa_up_1 + sa_df4 + sa_df3 + sa_df2 + sa_df1

        return decoded_f1, features_list, attn_list
    

class FFTAttentionUNetWithWavelets(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        y = self.in_conv(x)
        y, output_wavelets, sa_list = self.unet(y)
        y = self.out_conv(y)
        ...
        return x + y, output_wavelets, sa_list
...
if __name__ == '__main__':
    ...
    model = FFTAttentionUNetWithWavelets(3, 3)
