...
class InvTranspose(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        L = H * W
        x = x.contiguous().view(B, C, L).transpose(2, 1)
        return x
...
# For spatial process
class ForwardTranspose(nn.Module):
    def __init__(self):
        super().__init__()
...
def forward_transformer_transpose(x: torch.Tensor) -> torch.Tensor:
    B, C, H, W = x.shape
    L = H * W
    x = x.contiguous().view(B, C, L).transpose(2, 1)
    return x
...
def inv_transformer_transpose(x: torch.Tensor) -> torch.Tensor:
    B, L, C = x.shape
    H = int(math.sqrt(L))
    W = int(math.sqrt(L))
    x = x.transpose(1, 2).view(B, C, H, W)
