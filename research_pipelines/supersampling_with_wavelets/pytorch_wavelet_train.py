...
class CustomTrainingPipeline(object):
    def __init__(self,
        ...
        self.train_dataset = SuperSamplingDataset(
            train_data_path,
            self.image_shape[0],
            dataset_size=10000
        )
        self.val_dataset = SuperSamplingDataset(
            val_data_path,
            self.image_shape[0],
            dataset_size=32
        )
    ...
    def _train_step(self, epoch) -> float:
        ...
        with tqdm.tqdm(total=len(self.train_dataloader)) as pbar:
            for i, (_lr_image, _hr_image) in enumerate(self.train_dataloader):
                self.optimizer.zero_grad()

                lr_image = _lr_image.to(self.device)
                hr_image = _hr_image.to(self.device)
    ...
    def _validation_step(self) -> Tuple[float, float]:
        ...
        if self.val_dataloader is not None:
            for _lr_image, _hr_image in tqdm.tqdm(self.val_dataloader):
                with torch.no_grad():
                    lr_image = _lr_image.to(self.device)
                    hr_image = _hr_image.to(self.device)
                    wavelets_pred = self.model(lr_image)
