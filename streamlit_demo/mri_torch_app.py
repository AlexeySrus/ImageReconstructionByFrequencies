...
TRACED_MODEL_PATH: str = os.path.join(DIR_PATH, 'mri_tfaunet.pt')
IMAGE_SIZE: int = 256
DEVICE: str = 'cpu'
...
def window_denoise_inference(
    ...
    Args:
        model (torch.nn.Module): PyTorch traced model
        ...
        device (str, optional): Device to inference. Defaults to 'cpu'.
    ...
    # To 0..1 and torch.Tensor
    padded_tensor = torch.from_numpy(padded_image.astype(np.float32) / 255.0)
    ...
    for transform in transforms_loop_generator:
        ...
        with torch.no_grad():
            outs = torch.cat(
                [
                    model(bc)[:, :, crop_d:-crop_d, crop_d:-crop_d] \
                        if crop_d > 0 else \
                            model(bc)
                    for bc in crops
                ],
    ...
    result_image = torch.mean(predicted_images, dim=0).to('cpu').numpy()
...
@st.cache_resource()
def cached_model() -> torch.nn.Module:
    jit_model = torch.jit.load(TRACED_MODEL_PATH).to(DEVICE)
    jit_model.eval()
    return jit_model
